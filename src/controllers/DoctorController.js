...
DoctorController.listDoctors = async (req, res) => {
  const listDoctors = await Doctor.find().select({"createdAt":0 , "updatedAt":0});
...
DoctorController.updateDoctorById = async (req, res) => {
  const { name, fatherLastName, motherLastName, email, phone, dni, age } =
    req.body;
